...
**Files:**
- Modify: `src/proxy.ts`
- Modify: `src/proxy.test.ts` (existing file — discovered during Task 4 execution; it currently imports the now-removed `getAuthCookieValue`/`isAuthCookieValid` and must be rewritten alongside `proxy.ts` in this same task, not left broken)

**Interfaces:**
- Consumes: `getAuthCookieName`, `verifySessionToken`, `createSessionToken` from `src/lib/auth.ts` (Task 4).
...
- [ ] **Step 2: Rewrite `src/proxy.test.ts` for the new per-user token API**

The existing file (`src/proxy.test.ts`) calls `getAuthCookieValue()` — a function Task 4 removed. Replace the whole file:
...
Run `npm test -- proxy.test` and confirm all 8 tests pass.

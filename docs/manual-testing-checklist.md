...
- [ ] Native: pick an image offline and save, reopen the edit page (still offline), tap "Αφαίρεση εικόνας" and save again → reconnect → the FIRST edit's image still uploads and both edits apply, with NO needs-attention error in the sync badge (the remove must not delete the earlier queued update's local bytes).
